...
Item {
    ...
    Item {
        ...
        Canvas {
            ...
            onPaint: {
                ...
                for (var i = 0; i < numSectors; i++) {
                    ...
                    var isActive = false;
                    if (i === 0 && root.currentFormat === "numeric") isActive = true;
                    else if (i === 1 && root.currentFormat === "alpha") isActive = true;
                    else if (i === 2 && root.currentFormat === "roman") isActive = true;
        ...
        Connections {
            target: root
            function onCurrentFormatChanged() { radialCanvas.requestPaint(); }
        ...
        Repeater {
            model: [
                { icon: "looks_one", label: "Numeric", format: "numeric" },
                { icon: "title", label: "Alphabetic", format: "alpha" },
                { icon: "tag", label: "Roman", format: "roman" }
            ]
            ...
            delegate: Item {
                ...
                Column {
                    ...
                    DankIcon {
                        name: modelData.icon
                        size: 22
                        color: root.currentFormat === modelData.format ? Theme.onPrimary : Theme.surfaceVariantText
        ...
        MouseArea {
            ...
            onReleased: (mouse) => {
                ...
                if (root.selectedIndex === 0) {
                    root.formatSelected("numeric");
                } else if (root.selectedIndex === 1) {
                    root.formatSelected("alpha");
                } else if (root.selectedIndex === 2) {
                    root.formatSelected("roman");
                } else if (root.selectedIndex === -2) {
                    root.centerClicked();
                }
                root.selectedIndex = -1;
